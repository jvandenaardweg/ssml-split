...
const { SSMLParseError, ValidationError, NotPossibleSplitError } = require('./errors')
...
class SSMLSplit {
    ...
    split(ssml) {
        ...
        this._root.children.forEach(node => {
            console.log('iteration', this._batches.length)
    ...
    _splitTextNode(node) {
        ...
        const localSoftLimit =
            this._textLength === 0 ? this._softLimit : this._textLength - this._softLimit
